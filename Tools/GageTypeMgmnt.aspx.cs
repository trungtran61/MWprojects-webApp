...
{
    public partial class GageTypeMgmnt : Common.pgAbstract
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (loadSec("Tools/Gage Type Management"))
            {
            ...
            if (e.CommandName.Equals("AddNew") || e.CommandName.Equals("InsertNew"))
            {
                ...
                switch (iGV.ID)
                {
                    case "gvNameList":
                        ...
                        odsNameList.UpdateParameters["GageName"].DefaultValue = (Rw.FindControl("txtGageName") as TextBox).Text;
                    ...
                    case "gvUnitList":
                        ...
                        odsUnitList.UpdateParameters["GageUnitTxt"].DefaultValue = (Rw.FindControl("txtGageUnitTxt") as TextBox).Text;
                        odsUnitList.UpdateParameters["GageUnitVal"].DefaultValue = (Rw.FindControl("txtGageUnitVal") as TextBox).Text;
        ...
        {
            odsDmsList.InsertParameters["GageDmsIDs"].DefaultValue = clbDmsLib.SelectedValues;
            odsDmsList.Insert();
        ...
        protected void ddlSelected(object sender, EventArgs e)
        {
            ...
            if (lblNew.Visible)
            {
                lblNew.Text = string.Format("Click to add new gage to block for {0}", ddlVendorList.SelectedItem.Text);
