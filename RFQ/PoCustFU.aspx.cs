...
namespace webApp.RFQ
{
    public partial class PoCustFU : pgAbstract
    {
        ...
        {
            return string.Format("<a onclick=\"javascript:lPopup('../File/Preview.aspx?AppCode=RFQ&FID=PoCustFU&Code=PoCustFU&HID={0}&uID={2}'); return false;\" class=\"mLink\">{1}</a>",
                Eval("HID"), Eval("CompanyName"), Common.clsUser.uID);
        }
        protected string gNote()
        {
            return string.Format("<a onclick=\"javascript:xPopup('PoCustFU_Log.aspx?AppCode=RFQ&HID={0}&cNm={1}&vLog=1'); return false;\" class=\"mLink\">View</a>", Eval("HID"), Eval("CompanyName"));
        }
        protected void rwCmd(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                int LnkID = Convert.ToInt32(e.CommandArgument), GrpID = Convert.ToInt32(myBiz.Tools.clsUtils.myUtil.AppSetting("PoCustFURFQ"));
        ...
        protected void doRefresh(object sender, EventArgs e)
        {
            ...
            (new myBiz.DAL.clsPoCustFU()).PoCustFU_Status_U(v[0], v[1]);
            odsFoURecd.DataBind(); gvFoURecd.DataBind();
        }
        protected void saveRFQ(object sender, EventArgs e)
        {
            (new myBiz.DAL.clsPoCustFU()).PoCustFU_SaveRFQ(hfHID.Value, txtRFQ.Text);
            odsFoURecd.DataBind(); gvFoURecd.DataBind();
        }
        protected void doReady(object sender, EventArgs e)
        {
            ...
            if (x.Length > 0)
            {
                (new myBiz.DAL.clsPoCustFU()).PoCustFU_ReadyRFQ(x.ToString(), isReady ? "Ready" : "Uploaded");
